...
export default function ReportIssue() {
  ...
  const [coords, setCoords] = useState({ lat: 28.6692, lng: 77.4538 })
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (formGridRef.current) {
        tl.fromTo(
          formGridRef.current.children,
          { y: 25, opacity: 0 },
  ...
  return (
    <AppLayout title={t('navReport')}>
      <div ref={headerRef} className="mb-8 pb-6 border-b border-black/5 dark:border-white/10">
        <h1 className="font-serif text-3xl sm:text-4xl text-neutral-900 dark:text-white font-normal tracking-tight">
          {t('reportHeaderTitle')}
        </h1>
      ...
      <form ref={formGridRef} onSubmit={submit} className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <Card className="bg-white dark:bg-[#1A1C20] rounded-3xl p-7 shadow-soft border border-black/5 dark:border-white/10">
            <h2 className="font-serif text-2xl text-neutral-900 dark:text-white font-bold mb-4">{t('step1Title')}</h2>
          ...
          <Card className="bg-white dark:bg-[#1A1C20] rounded-3xl p-7 border border-black/5 dark:border-white/10 shadow-soft space-y-4">
            <div className="flex items-center justify-between mb-2">
              <h2 className="font-serif text-2xl text-neutral-900 dark:text-white font-bold">{t('step2Title')}</h2>
              {aiResult && (
                <button
                  type="button"
                  onClick={handleAiAutoFill}
                  className="text-xs font-bold px-3.5 py-1.5 rounded-full bg-gradient-to-r from-blue-600 to-violet-600 text-white flex items-center gap-1.5 shadow-sm hover:scale-[1.02] transition-transform cursor-pointer"
                >
            ...
            <div>
              <label className="text-xs font-bold uppercase tracking-wider text-neutral-500 dark:text-neutral-400 mb-2 block">{t('titleLabel')}</label>
              <input
                type="text"
                required
                value={title}
                ...
                className="w-full rounded-2xl bg-neutral-100/90 dark:bg-neutral-800/60 border border-transparent focus:border-blue-500 focus:bg-white dark:focus:bg-neutral-800 px-4 py-3.5 text-sm text-neutral-900 dark:text-white font-bold outline-none transition-all"
              />
            </div>

            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-bold uppercase tracking-wider text-neutral-500 dark:text-neutral-400 block">{t('descLabel')}</label>
                <span className="text-[11px] text-neutral-400 font-medium">Editable AI Generated Draft</span>
              </div>
              <textarea
                required
                rows={4}
                ...
                className="w-full rounded-2xl bg-neutral-100/90 dark:bg-neutral-800/60 border border-transparent focus:border-blue-500 focus:bg-white dark:focus:bg-neutral-800 p-4 text-sm text-neutral-900 dark:text-white outline-none resize-none transition-all"
              />
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card className="bg-white dark:bg-[#1A1C20] rounded-3xl p-7 border border-black/5 dark:border-white/10 shadow-soft">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-serif text-2xl text-neutral-900 dark:text-white font-bold flex items-center gap-2">
            ...
            <MapCard
              ...
              height="h-48"
            />
            <p className="text-xs font-bold text-neutral-800 dark:text-neutral-200 mt-3.5 leading-relaxed">{locationAddress}</p>
          </Card>

          {duplicateWarning && (
            <div className="rounded-3xl bg-amber-50 dark:bg-amber-950/40 p-5 border border-amber-200 dark:border-amber-900 text-amber-900 dark:text-amber-200 flex items-start gap-3 text-xs leading-relaxed shadow-sm">
          ...
          <AnimatePresence mode="wait">
            {analyzing && (
              <motion.div
                ...
                exit={{ opacity: 0 }}
                className="rounded-3xl bg-[#E7F1FE] dark:bg-[#122238] p-8 text-center space-y-4 border border-blue-200 dark:border-blue-900"
              >
                <Loader2 size={32} className="animate-spin text-blue-600 dark:text-blue-400 mx-auto" />
                <h3 className="font-serif text-2xl text-blue-950 dark:text-blue-100 font-bold">AI Computer Vision Audit in Progress...</h3>
                <p className="text-xs text-blue-800 dark:text-blue-300 font-medium">Uploading photo & analyzing issue category & severity</p>
              </motion.div>
            )}
            ...
            {wasteResult && !analyzing && (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono font-bold uppercase px-2.5 py-1 rounded-full bg-blue-500/15 text-blue-700 dark:text-blue-300 border border-blue-500/20">
                    YOLOv8 Detected
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="p-3.5 rounded-2xl bg-white/80 dark:bg-black/30 border border-black/5 dark:border-white/10">
                    <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block">Primary Category</span>
                  ...
                  <div className="p-3.5 rounded-2xl bg-white/80 dark:bg-black/30 border border-black/5 dark:border-white/10">
                    <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block">Recommended Bin</span>
              ...
              <motion.div
                ...
                className={`rounded-3xl p-7 space-y-4 shadow-soft border ${
                  aiResult.isCivicIssue === false
                    ...
                    : 'bg-[#E4F8EE] dark:bg-[#0F2D20] border-emerald-300/60 dark:border-emerald-900/40'
                }`}
              >
                <div className="flex items-center justify-between">
          ...
          <Button type="submit" disabled={submitting || uploadingImage} className="w-full justify-center shadow-lg text-base py-3.5">
